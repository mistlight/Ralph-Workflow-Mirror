...
//!
//! This module contains the `execute_mock` method, which maps each `Effect` variant
//! to appropriate mock `PipelineEvent`(s) without performing real side effects.
//!
//! ## Purpose
//!
...
//! ## Design
//!
//! This is essentially a large match statement mapping effects to events.
//! Each match arm returns:
//! - A primary `PipelineEvent` describing the effect outcome
//! - Optional `UIEvent`s for display (not affecting correctness)
//! - Optional additional `PipelineEvent`s for multi-step effects
//!
//! ## Special Cases
//!
//! Effects requiring workspace access (`SaveCheckpoint`, `TriggerDevFixFlow`)
//! are NOT handled here - they're handled in the `EffectHandler::execute()`
