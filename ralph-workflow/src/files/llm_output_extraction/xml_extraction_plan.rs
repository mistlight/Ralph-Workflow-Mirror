...
//!
//! This module provides robust extraction of XML plans from various
...
/// Extract XML plan from AI output using multiple strategies.
///
/// # Strategies (tried in order)
///
/// 1. **Direct extraction**: Content starts with `<ralph-plan>` tag
/// 2. **Markdown code fence**: XML wrapped in ```xml or ``` fences
/// 3. **JSON string**: XML escaped in a JSON string value
/// 4. **Embedded search**: Look for `<ralph-plan>` anywhere in content
///
/// # Arguments
///
...
/// Strategy 1: Extract XML that starts with `<ralph-plan>` tag.
fn try_extract_direct_xml(content: &str) -> Option<String> {
    ...
    let start = trimmed.find("<ralph-plan>")?;
    let end = trimmed.find("</ralph-plan>")?;
...
fn try_extract_from_markdown_fence(content: &str) -> Option<String> {
    ...
    if let Some(start) = content.find("```xml") {
        ...
        if let Some(end) = after_fence.find("```") {
            ...
            if let Some(xml) = extract_ralph_plan_from_content(fence_content) {
                return Some(xml);
            }
        }
    }
    ...
    for line in content.lines() {
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(trimmed) {
            if let Some(result) = json.get("result").and_then(|v| v.as_str()) {
                if let Some(xml) = extract_ralph_plan_from_content(result) {
                    return Some(xml);
                }

                let unescaped = unescape_json_strings_aggressive(result);
                if let Some(xml) = extract_ralph_plan_from_content(&unescaped) {
                    return Some(xml);
                }
            }
            ...
            for field_name in ["content", "message", "output", "text"] {
                if let Some(field_value) = json.get(field_name).and_then(|v| v.as_str()) {
                    if let Some(xml) = extract_ralph_plan_from_content(field_value) {
                        return Some(xml);
                    }
...
fn try_extract_embedded_xml(content: &str) -> Option<String> {
    extract_ralph_plan_from_content(content)
}

/// Extract `<ralph-plan>...</ralph-plan>` from arbitrary content.
fn extract_ralph_plan_from_content(content: &str) -> Option<String> {
