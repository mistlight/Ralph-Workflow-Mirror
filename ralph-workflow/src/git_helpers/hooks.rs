...
/// Check if a hook file has been tampered with or bypassed.
///
...
/// filesystem errors.
///
/// A hook is considered tampered with if:
/// - The hook file doesn't exist but is expected to
/// - The hook exists but doesn't contain the Ralph marker
/// - The hook exists but has been modified since installation (not currently detected)
///
/// This is a best-effort check and doesn't provide complete security guarantees.
#[cfg(test)]
pub fn verify_hook_integrity(hook_path: &Path) -> io::Result<bool> {
    if !hook_path.exists() {
        return Ok(false);
    }
    file_contains_marker(hook_path, HOOK_MARKER)
}
...
mod tests {
    ...
    // =========================================================================
    // Original tests using TempDir (for verify_hook_integrity - uses absolute paths)
    // =========================================================================

    #[test]
    fn test_verify_hook_integrity_missing() {
        let temp = tempfile::TempDir::new().unwrap();
        let missing_hook = temp.path().join("pre-commit");
        assert!(!verify_hook_integrity(&missing_hook).unwrap());
    }

    #[test]
    fn test_verify_hook_integrity_valid_ralph_hook() {
        let temp = tempfile::TempDir::new().unwrap();
        let hook_path = temp.path().join("pre-commit");

        // Write a valid Ralph hook
        let hook_content = format!(
            r#"#!/usr/bin/env bash
# {HOOK_MARKER} - generated by ralph
...
"#,
        ...
        assert!(verify_hook_integrity(&hook_path).unwrap());
    }

    #[test]
    fn test_verify_hook_integrity_tampered_hook() {
        let temp = tempfile::TempDir::new().unwrap();
        let hook_path = temp.path().join("pre-commit");

        // Write a hook without the Ralph marker (simulating tampering)
...
"#;
    ...
    #[test]
    fn test_verify_hook_integrity_modified_marker() {
        let temp = tempfile::TempDir::new().unwrap();
        let hook_path = temp.path().join("pre-commit");

        // Write a hook with a completely different marker (simulating bypass attempt)
        let hook_content = r#"#!/usr/bin/env bash
# NOT_RALPH_MANAGED_HOOK - tampered marker
echo "Tampered hook"
exit 0
"#;
        fs::write(&hook_path, hook_content).unwrap();
