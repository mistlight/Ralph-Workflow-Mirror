...
fn ralph_checkpoint_contains_iteration_info() {
    with_default_timeout(|| {
        ...
        // Pre-create a checkpoint file with expected structure
        // This tests checkpoint validation and structure without requiring agent execution
        fs::create_dir_all(dir.path().join(".agent")).unwrap();
...
fn ralph_checkpoint_contains_cli_args_snapshot() {
    with_default_timeout(|| {
        ...
        // Pre-create a checkpoint file with CLI args snapshot
        fs::create_dir_all(dir.path().join(".agent")).unwrap();
        let working_dir = canonical_working_dir(&dir);
        fs::write(
            dir.path().join(".agent/checkpoint.json"),
...
fn ralph_checkpoint_contains_agent_config_snapshot() {
    with_default_timeout(|| {
        ...
        // Pre-create a checkpoint file with agent config snapshot
        fs::create_dir_all(dir.path().join(".agent")).unwrap();
        let working_dir = canonical_working_dir(&dir);
        fs::write(
            dir.path().join(".agent/checkpoint.json"),
