...
fn make_dylint_target_forces_nightly_cargo_resolution() {
    with_default_timeout(|| {
        ...
        assert!(
            makefile.contains("rustup which cargo --toolchain \"$$NIGHTLY_TOOLCHAIN\""),
        ...
        assert!(
            makefile.contains("export RUSTUP_TOOLCHAIN=\"$$NIGHTLY_TOOLCHAIN\""),
            "dylint target should export RUSTUP_TOOLCHAIN to nightly toolchain"
        );
