...
fn ralph_reset_start_commit_flag_works() {
    ...
    test_support::commit_all(&repo, "second commit");

    // Get the current HEAD commit OID
    let head_oid_str = test_support::head_oid(&repo);
...
"#,
    ...
    // Verify the change files exist (changes were created)
    assert!(dir.path().join("iteration_1.txt").exists());
    assert!(dir.path().join("iteration_2.txt").exists());

    // Verify the developer ran 4 times (2 iterations × 2 phases)
...
fn ralph_reviewer_receives_cumulative_diff_from_start() {
    ...
    test_support::commit_all(&repo, "baseline");
...
"#,
    ...
    let first_start_commit = fs::read_to_string(dir.path().join(".agent/start_commit")).unwrap();

    // Create a new commit
    fs::write(dir.path().join("new_file.txt"), "content").unwrap();
    let repo = git2::Repository::open(dir.path()).unwrap();
    test_support::commit_all(&repo, "new commit");
    ...
    let second_start_commit = fs::read_to_string(dir.path().join(".agent/start_commit")).unwrap();
    assert_eq!(
