...
use crate::git_helpers::{commit_with_auto_message_fallback_result, get_git_diff_from_start, git_snapshot};
    ...
    for j in start_pass..=ctx.config.reviewer_reviews {
        ...
        if review_prompt.is_empty() {
            ...
            if snap != prev_snap {
                ...
                // Get the commit agent chain for fallback-aware commit generation
                let commit_agent_chain = get_commit_agent_chain(ctx);
                if !commit_agent_chain.is_empty() {
                    ctx.logger.info(&format!("Creating commit with auto-generated message ({} agent(s) in fallback chain)...", commit_agent_chain.len()));
